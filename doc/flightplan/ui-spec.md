...
**壳层**（决策 32/50/58 修订）：最左侧垂直图标栏（w-14，毛玻璃浮层）。Phase 9 模块 = 飞行计划（激活）+ 航图/设置灰显占位；**当前模块高亮指示（G6）**；图标栏左下角 = **设置入口**（Phase 9 灰显占位）。

**地图全屏背景**（决策 50）：地图绝对定位铺满整个应用背景；左侧任务区（w-96）与图标栏浮于其上，**毛玻璃**（`bg-background/75 backdrop-blur-md` + 细边框，暗色独立底色 token）。地图/任务区比例不变。

```text
┌──┬──────────────────────────────────────────────────┐
│功│ 地图（全屏背景，absolute inset-0）                │
│能│ ┌──────────────────┐          ┌───────────────┐   │
│栏│ │ 任务区（毛玻璃）   │          │ [Layers ▾]   │   │ ← 右上圆角竖栏
│  │ │ 顶部双页签        │          │ [主题三态]     │   │（决策 58）
│  │ │ ① 表单           │          └───────────────┘   │
│  │ │ ③ Flight Task    │  底图 + 计划航路 + 候选灰显   │
│  │ └──────────────────┘                              │
│⚙│                                                    │
└──┴──────────────────────────────────────────────────┘
```

**响应式**（决策 32/60 修订）：Tauri minWidth/minHeight 900×600；CSS 断点（长边 ~1000px）以下降级为横菜单栏 + **任务区/地图纵向堆叠**（决策 60：任务区全宽浮动毛玻璃面板 max-h-[55%] + 内部滚动在上，地图区 min-h-[240px] 在下；工具竖栏挂地图区右上），降级激活时顶部横幅提醒（主要为将来 Web 部署兜底）。

**顶部双页签**（决策 52）：任务区顶端 **本地填写** / **SimBrief 导入** 两个 tab（图标 + 文字）。

**两阶段交互流**（决策 56，决策 34 修订）：① 表单（含 Route 分区 + Suggest Route 内嵌候选选择）→ ③ 生成后 Flight Task 面板；② 候选列表阶段废弃。生成计划按钮位于表单底部（Route 有已 analyzed 航路才可用）。返回按钮 = 表单 ↔ Task 互切。

## 3. 生成前表单（本地填写页签）

**折叠策略**（决策 53）：核心分区 Flight Info / Aircraft / Selections / Route / Suggest Route **固定展开**；仅 Optional Entries / Fuel Planning 折叠（渐进披露）。

| 分区 | 内容 |
|---|---|
| Flight Info | 呼号（callsign，前端自持显示）、起降场、ETD（决策 10 修订；**跑道字段移除——决策 57，TODO(Phase 11 METAR)**） |
...
| Optional Entries | 途经点/避让点/航路规则等（决策 7；折叠） |
| Fuel Planning | 燃油政策预设 + 额外油 + 滑行时间（决策 11/21；数值 Phase 10 填充，表单先立住；折叠） |
| Route | **大输入框**（placeholder 浅字"在此处输入您的航路"，决策 54）+ 左侧 **Analyze Route 按钮** + 右侧**合法性信息框**（成功：`在 AIRAC xxxx 数据下验证成功，航路距离 xxx nm`；失败：逐条列出不存在点/航路）。Analyze 后航路成为**计划航路**（地图高亮，生成按钮解锁）——`plan.analyze` 新端点 |
| Suggest Route | **偏好选择框（后端 profile，初始 NONE/无）+ 生成按钮 + 5 个横排候选框**（决策 55 修订，plan.routes k=5）。**全部候选线常驻地图灰显**；hover 候选框 → 该候选高亮；点击 → 回填 Route 输入框；Route 已有计划航路时点候选 → **替换确认 Dialog**（丢失分析结果警告）。**候选与 Route 计划航路一致 → 蓝色提示框"Route输入航路"** |

**SimBrief 导入页签**（决策 52）：仅两元素——pilotid 输入（SimBrief API 测试）+ 导入按钮。导入成功 → 自动 analyze → 取首候选 → generate（配载默认显式 pax=0/cargo=0）→ **直接跳 Task 节点**；失败 → Dialog 错误窗口（pilotid 无效 / API 无航班计划 / 网络失败 / 解析失败，附常见原因注释）。粘贴 HTML 导入保留（DOMParser，决策 39）。

表单模式：内联校验（airframe 校验错误显示在字段旁，**onBlur 触发 + `role="alert"`/`aria-live` 播报，G7/G8**）、渐进披露（高级设置折叠）、AUTO 联动项。**Route 无已 analyzed 航路 → 生成计划按钮禁用**。
...
| 层 | Phase | 数据 |
|---|---|---|
| 底图瓦片 | 9 | 可换源（随主题）；**恒显不可关（决策 51）** |
| 计划航路层 | 9 | `plan.analyze` 结果（主色高亮，决策 54） |
| 候选航路层 | 9 | plan.routes points，**全部候选常驻灰显**；hover 候选框 → 该候选高亮；已 analyzed 计划航路（主色）与候选严格分色；与计划航路一致的候选 → 蓝色提示框（决策 55 修订） |
| 航路点/机场标注 | 9 | lookup 透传；**Phase 9 全部同色**（点类型着色跳过，决策 41）；**标签常显分级（决策 59）**——起降场/备降场 12px 常显(zoom≥5)、计划航路中间点 10px 常显(zoom≥7)、候选点 hover 临时显示 |
| 备降机场标记 | 9 | plan.alternates（lat/lon） |
...
契约：每层 = 独立 React 组件 + 显式开关（**地图右上圆角竖栏 Layers 下拉菜单，决策 58**）+ 显隐状态持久化（localStorage）；层间不互知，仅通过 LayerManager 注册。竖栏含 Layers / 主题三态两按钮（主题从图标栏迁入）。
...
## 6. 交互流（对齐两步式 + 两阶段，决策 56）

本地页签：设置表单 → Route 分区 [Analyze Route]（plan.analyze，合法性 + 距离 + 地图显示计划航路）→ 或 Suggest Route 生成候选（偏好 profile + plan.routes → 5 候选框 → hover 灰显 → 点击回填 Route → 替换确认）→ [生成计划]（plan.generate，Route 有已 analyzed 航路才可用）→ Flight Task 面板 + 警告条 → [Download .PLN] / [Prefile] / [导出调度单 TODO]。SimBrief 页签：pilotid + [导入] → 自动 analyze → 首候选 → generate（配载默认 0）→ 直接 Task；失败 → Dialog。返回按钮 = 表单 ↔ Task 互切。候选过期提示（参数改后候选过期 → 警告条橙黄；计划航路 analyze 结果与当前数据周期不一致时同样提示）。
...
- D16（ui grill 2，决策 50-58）：地图全屏背景 + 毛玻璃浮层（D50）；Basemap 层删除恒显（D51）；顶部双页签本地/SimBrief（D52，SimBrief 直跳 Task + 失败 Dialog）；折叠策略修正——核心分区固定展开（D53）；Route 大输入框 + Analyze Route + 合法性信息框 + **新端点 plan.analyze**（D54）；Suggest Route 栏目（后端 profile 偏好 + 5 候选框 + hover 灰显 + 替换确认）（D55）；**三子阶段 → 两阶段**（D56，②候选列表废弃）；起降跑道移除 TODO METAR（D57）；地图右上圆角竖栏 Layers 下拉 + 主题，设置入口左下角（D58）。
