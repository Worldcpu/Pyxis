...
| 50 | **地图全屏布局（决策 32 修订，2026-08-07 ui grill 2）**：地图绝对定位铺满整个应用背景（`absolute inset-0`）；左侧任务区（w-96）与图标栏（w-12→**w-14** 微扩）浮于其上，**毛玻璃**（`bg-background/75 backdrop-blur-md` + 细边框）。地图/任务区比例不变。暗色毛玻璃底色独立 token。WebKit 性能兜底：卡顿降级 `bg-background/90` 无 blur。 |
| 51 | **Basemap 层删除（决策 40 图层系统修订，2026-08-07 ui grill 2）**：底图瓦片恒显不可关（关了看不见地图，无实际意义）——LayerManager 移除 basemap 开关，图层面板仅候选航路/航路点/备降机场 3 层。 |
| 52 | **顶部双页签（2026-08-07 ui grill 2）**：任务区顶端两个 tab（图标+文字）：**本地填写** / **SimBrief 导入**。SimBrief 页仅两元素：pilotid 输入（SimBrief API 测试）+ 导入按钮。**导入成功 → 自动 analyze → 取首候选 → generate（配载默认显式 pax=0,cargo=0）→ 直接跳 Task 节点**（决策 34 生成后节点）；失败 → Dialog 错误窗口，错误分类（pilotid 无效/API 无航班计划/网络失败/解析失败）附常见原因注释。粘贴 HTML 导入仍保留（DOMParser，决策 39）。 |
| 53 | **折叠策略修正（决策 40 修订，2026-08-07 ui grill 2）**：核心分区 **Flight Info / Aircraft / Selections / Route / Suggest Route 固定展开**（不可折叠）；仅渐进披露分区折叠——Optional Entries / Fuel Planning（表单）+ Prefile / Download（Task，决策 38/17 默认折叠）。 |
| 54 | **Route 页改造 + 航路解析（2026-08-07 ui grill 2）**：Route 分区 = 大输入框（placeholder 浅字"在此处输入您的航路"）+ 左侧 **Analyze Route 按钮** + 右侧**合法性信息框**。成功：`在 AIRAC xxxx 数据下验证成功，航路距离 xxx nm`；失败：逐条列出（`不存在航路点 XXX` / `航路 XXX 不存在`）。**新增端点 `plan.analyze`**：route_string → {valid, cycle, distance_nm, errors[]}（复用 bf ParseRoute + 逐段校验 + 距离）。Analyze 后航路成为**计划航路**（地图高亮 + 表单底部生成按钮解锁）；输入框无航路 → 生成禁用。 |
| 55 | **Suggest Route 栏目（2026-08-07 ui grill 2，修订：hover 行为 + 匹配提示）**：表单页新分区。顶部 = **偏好选择框（后端 profile）** + 生成按钮；下方 5 个横排候选框（plan.routes k=5，SimBrief 式）。**全部候选线常驻地图灰显**；hover 候选框 → 该候选**高亮**（灰 → 高亮色）。点击候选 → 回填 Route 输入框；Route 已有计划航路时点候选 → **替换确认 Dialog**（提示丢失当前分析结果）。**候选与 Route 计划航路一致 → 该候选框蓝色提示框标注"Route输入航路"**（蓝色 = 选中航路色）。偏好初始 NONE/无（不应用偏好，用表单当前参数）。 |
| 56 | **两阶段交互流（决策 34 修订，2026-08-07 ui grill 2）**：**② 候选列表阶段废弃**——三子阶段 → 两阶段（① 表单（含 Suggest Route 内嵌候选选择）→ ③ Task）。生成计划按钮移至表单底部（Route 有已 analyzed 航路即可点）。返回按钮逻辑简化（表单 ↔ Task）。 |
| 57 | **起降跑道字段移除（决策 10 修订，2026-08-07 ui grill 2）**：Flight Info 去掉起降跑道输入——跑道选择属于 METAR 数据落地后的能力，留 **TODO(Phase 11 METAR)**；后端 plan.routes 的 departure_runway/arrival_runway 参数保留（API 层兼容，前端不传）。 |
| 58 | **地图右上工具竖栏 + 设置入口（决策 33/40 修订，2026-08-07 ui grill 2）**：地图右上角**圆角竖栏**（rounded-lg border + 毛玻璃）两枚按钮：**Layers**（点击弹下拉菜单：候选航路/航路点/备降机场 3 开关）与**主题三态**（从图标栏左下角迁入）。图标栏左下角改为**设置入口**（Phase 9 灰显占位）。 |
| 59 | **航路点标签常显（决策 41 修订，2026-08-07 ui grill 3）**：SimBrief 式常驻点标签。分级：**起降场 + 备降场标签常显**（12px 加粗，zoom ≥ 5）；**计划航路中间点标签常显**（10px，zoom ≥ 7）；**候选灰线点标签默认不显**——hover 候选框 → 该候选高亮 + 点标签临时显示（zoom ≥ 7）。样式：纯文字无背景 + text-shadow 描边（随主题反色）、`direction: top` 点上方、Fira Code、**pointer-events-none + interactive false**（不挡点点击/地图拖拽）。实现：Tooltip permanent + useMap zoomend 分级条件渲染。 |
| 60 | **窄窗口纵向堆叠（决策 50 修订，2026-08-07 ui grill 3）**：< lg 窄窗口任务区与地图**上下堆叠**——任务区 = 全宽浮动毛玻璃面板（`max-h-[55%]` + 内部滚动，决策 50 毛玻璃语义统一保留）在上，地图区（`min-h-[240px]` 剩余空间）在下；宽窗口（≥ lg）保持现状（任务区 w-96 左侧浮动 + 地图弹性）。**工具竖栏（Layers/主题）挂在地图区容器右上**（绝对定位）——窄窗口下自然落在地图区、不被面板遮盖；Layers 下拉窄窗口收窄 w-36。实现纯 CSS 断点（flex-col lg:flex-row + max-h 切换，无 JS matchMedia）；Tauri min 900×600 天然命中窄布局。 |
