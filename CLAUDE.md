...
- C++17、CMake 3.26+、Google C++ Style（clang-format）
...
1. `lib/` 必须保持**纯 C++17，严禁引入 JSON、网络和 UI 依赖**。只依赖标准库和通过 FetchContent 获取的 `third_party/` 库。
...
- 现代 C++17；**Google C++ Style**（格式化基础，clang-format `BasedOnStyle: Google`）
...
- 错误处理使用 `px::Result<T, E>`（封装 `tl::expected`）；预期失败（找不到航路、数据缺失）通过 `Result` 传递，异常仅用于真正异常的情况。
...
- WebSocket 通信：React 前端与 C++ 后端通过 `ws://127.0.0.1:port` 直连。
- `tauri.conf.json` 中的 `csp` 必须显式配置 `connect-src 'self' ws://127.0.0.1:* ws://localhost:*`。
- **动态端口与安全鉴权**：Tauri 启动 C++ 子进程时分配随机可用端口并传入 `--port` 和 `--token <SECRET>`；React 启动时通过 Tauri IPC 获取端口和 Token 后发起 WebSocket 连接。
